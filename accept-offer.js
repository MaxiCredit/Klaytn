const acceptCurrentOffer = require('./accept-credit-offer-mn');
...
exports.acceptCreditOffer = function(req, res) {
    ...
    req.on('end', function() {
		...
		creditId = offerData[1].slice(offerData[1].indexOf('=') + 1);
		creditAmount = offerData[2].slice(offerData[2].indexOf('=') + 1);
		privateKey = offerData[3].slice(offerData[3].indexOf('=') + 1);
		res.write(creditAmount + " " + creditId + " " + payback + " ");
		
		approve.approveMX('0x4Cf11FF20BCC43DFcafBcD0C17a53B98dFb27cEd', mxAbi, '0x2593979b76df2d2f8de2bf62f199bcb530755e3e', payback, privateKey, (err, result) => {
			if(!err) {
	...
	function accept() {
		acceptCurrentOffer.acceptOffer(creditId, creditAmount, privateKey, creditAbi, (err, result) => {
			if(!err) {
				res.end(result);
			} else {
